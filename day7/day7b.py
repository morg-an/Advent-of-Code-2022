...
def getTotalSize(directory):
    ...
    if len(children) == 0:
        ...
        childFileSize = findChildDirectorySize(directory)
        totalSize = size + childFileSize
        return totalSize

def getAnswer():
    root = Folder._registry[0]
    print(root.get_name())
    rootSize = getTotalSize(root)
    minDeleteableSize = rootSize-40000000
    print(f"Root Size: {rootSize}  Delete Minimum: {minDeleteableSize}")
    deleteableFiles = []
    for folder in Folder._registry:
        totalSize = getTotalSize(folder)
        if totalSize >= minDeleteableSize:
            deleteableFiles.append(totalSize)
    print(deleteableFiles)
    return min(deleteableFiles)
